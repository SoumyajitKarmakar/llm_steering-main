...
def compute_save_directions(llm, dataset, concept, control_method='rfm', path='directions/'):
    ...
    file_path = f'{path}{control_method}_{concept}_{llm.name}.pkl'
...
def main():
    ...
    save_path = 'directions_moods_plus_llama70/'
    ...
    for f_idx, fname in enumerate(fnames):
        ...
        for concept_idx, concept in enumerate(tqdm(concepts)):
            ...
            if dataset_label == 'fears':
                ...
                dataset = utils.pca_mood_dataset(llm, concept) # orig, disabled for qwen
                # dataset = utils.pca_mood_dataset_qwen(llm, concept, think=False)
